...
import { StorageCosts } from "@/types/synapse";
...
export const getPricePerTBPerMonth = (storageCosts: StorageCosts): bigint => {
  return config.withCDN ? storageCosts.pricePerTiBPerMonthWithCDN : storageCosts.pricePerTiBPerMonthNoCDN;
};
 ...
 */
export const calculateRateAllowanceGB = (rateAllowance: bigint, storageCosts: StorageCosts): number => {
  ...
  const bytesThatCanBeStored = (monthlyRate * SIZE_CONSTANTS.TiB) / getPricePerTBPerMonth(storageCosts);
  // Convert bytes to GB
