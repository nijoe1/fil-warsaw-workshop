...
import { StorageCalculationResult } from "@/types/synapse";
import {
  calculateCurrentStorageUsage,
  fetchWarmStorageBalanceData,
  fetchWarmStorageCosts,
} from "@/utils/synapse/warmStorageUtils";
import { SIZE_CONSTANTS, Synapse, TIME_CONSTANTS } from "@filoz/synapse-sdk";
...
export const calculateStorageMetrics = async (
  ...
  storageCapacityBytes: number = config.storageCapacity * Number(SIZE_CONSTANTS.GiB),
  ...
  const warmStorageBalance = await fetchWarmStorageBalanceData(synapse, storageCapacityBytes, persistencePeriodDays);
  ...
  const lockupPerDayAtCurrentRate = TIME_CONSTANTS.EPOCHS_PER_DAY * warmStorageBalance.currentRateUsed;

  // Calculate remaining lockup and persistence days
  const currentLockupRemaining = warmStorageBalance.currentLockupAllowance - warmStorageBalance.currentLockupUsed;
  ...
  const { currentStorageBytes, currentStorageGB } = calculateCurrentStorageUsage(
    warmStorageBalance,
    storageCapacityBytes,
  );

  // Determine sufficiency of allowances
  const isRateSufficient = warmStorageBalance.currentRateAllowance >= rateNeeded;
  ...
  const currentRateAllowanceGB = calculateRateAllowanceGB(warmStorageBalance.currentRateAllowance, storageCosts);
