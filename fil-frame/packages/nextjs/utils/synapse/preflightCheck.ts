...
export const preflightCheck = async (
  ...
  const { isSufficient, rateAllowanceNeeded, lockupAllowanceNeeded, depositAmountNeeded } = await checkAllowances(
    warmStorageBalance,
  ...
  if (!isSufficient) {
    ...
    const depositTx = await synapse.payments.deposit(depositAmountNeeded, "USDFC", {
