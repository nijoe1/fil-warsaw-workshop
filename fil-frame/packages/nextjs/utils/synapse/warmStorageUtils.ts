...
import { StorageCosts, WarmStorageBalance } from "@/types/synapse";
import { DATA_SET_CREATION_FEE } from "@/utils/synapse/constants";
import { SIZE_CONSTANTS, Synapse, TIME_CONSTANTS, WarmStorageService } from "@filoz/synapse-sdk";
 ...
 */
export const fetchWarmStorageCosts = async (synapse: Synapse): Promise<StorageCosts> => {
...
export const calculateCurrentStorageUsage = (
  ...
  if (warmStorageBalance.currentRateUsed > 0n && warmStorageBalance.rateAllowanceNeeded > 0n) {
    try {
      // Proportionally calculate storage usage based on rate used
      currentStorageBytes =
        (warmStorageBalance.currentRateUsed * BigInt(storageCapacityBytes)) / warmStorageBalance.rateAllowanceNeeded;
      // Convert bytes to GB
      currentStorageGB = Number(currentStorageBytes) / Number(SIZE_CONSTANTS.GiB);
