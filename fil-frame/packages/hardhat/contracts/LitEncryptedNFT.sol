...
contract LitEncryptedNFT is ERC721 {
	...
	function grantAccess(uint256 tokenId, address[] memory accessList) public {
		require(
			...
			"LitEncryptedNFT: Only owner can grant access"
		);
		// Grant access to the specified addresses
		for (uint i = 0; i < accessList.length; i++) {
			tokenContentAccess[tokenId][accessList[i]] = true;
	...
	function revokeAccess(uint256 tokenId, address[] memory accessList) public {
		require(
			ownerOf(tokenId) == msg.sender,
			"LitEncryptedNFT: Only owner can revoke access"
		);
	...
	function hasAccess(
		...
		return
			openAccessTokens[tokenId] || tokenContentAccess[tokenId][account];
	}
	...
	// Function to get all token details for an account
	function getAllTokensDetails(
		address account
	) public view returns (TokenDetails[] memory) {
		TokenDetails[] memory tokenDetails = new TokenDetails[](
			_tokenIdCounter
		);
		for (uint i = 0; i < _tokenIdCounter; i++) {
			tokenDetails[i].tokenPieceCid = _tokenURIs[i + 1];
			tokenDetails[i].hasAccess = hasAccess(i + 1, account);
		}
		return tokenDetails;
	}
}
